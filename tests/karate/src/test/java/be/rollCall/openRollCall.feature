...
Feature: Roll Call Open
  ...
  Scenario: Open a valid Roll Call
    ...
    And string rollCallOpen = converter.publishМessageFromData(rollCallOpenData, openRollCallId, laoChannel)
    ...
    * karate.log(open_roll)
    Then match open_roll contains deep {jsonrpc: '2.0', id: '#(openRollCallId)', result: 0}
  ...
  Scenario: Opening a Roll Call that does not exist should return an error
    ...
    And string rollCallOpen = converter.publishМessageFromData(badRollCallOpenData, openRollCallId, laoChannel)
    * call read('classpath:be/utils/simpleScenarios.feature@name=valid_lao')
    When eval frontend.send(rollCallOpen)
    * json err_open = frontend_buffer.takeTimeout(timeout)
    Then match err_open contains deep {jsonrpc: '2.0', id: '#(openRollCallId)', error: {code: -4, description: '#string'}}
  ...
  Scenario: Opening a Roll Call with invalid update_id should return an error
    ...
    And string badRollCallOpen = converter.publishМessageFromData(badRollCallOpenData, openRollCallId, laoChannel)
    * call read('classpath:be/utils/simpleScenarios.feature@name=valid_roll_call')
