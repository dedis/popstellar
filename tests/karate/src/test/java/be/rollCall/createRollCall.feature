...
Feature: Create a Roll Call
  ...
  Scenario: Valid Roll Call
    ...
    And string rollCallCreate = converter.publishМessageFromData(rollCallData, rollCallCreateId, laoChannel)
    ...
    * json roll_call_result = frontend_buffer.takeTimeout(timeout)
    Then match roll_call_result contains deep {jsonrpc: '2.0', id: '#(rollCallCreateId)', result: 0}
  ...
  Scenario: Roll Call Creation with empty name should return an error code
    ...
    * json roll_err = frontend_buffer.takeTimeout(timeout)
    Then match roll_err contains deep {jsonrpc: '2.0', id: '#(rollCallCreateId)', error: {code: -4, description: '#string'}}
  ...
  Scenario: Roll Call Creation with non-organizer as sender should return an error
    ...
    * string badRollCallCreate = converter.publishМessageFromData(badRollCallData, rollCallCreateId, laoChannel)
  ...
  Scenario: Roll Call Creation sent on root channel should return an error
    ...
    * string badRollCallCreate = converter.publishМessageFromData(badRollCallData, rollCallCreateId, rootChannel)

    When eval frontend.send(badRollCallCreate)
    * json roll_err = frontend_buffer.takeTimeout(timeout)
    Then match roll_err contains deep {jsonrpc: '2.0', id: '#(rollCallCreateId)', error: {code: -6, description: '#string'}}
  ...
  Scenario: Roll Call Creation with proposed start > proposed end should return and error
    ...
    And string badRollCallCreate = converter.publishМessageFromData(badRollCallData, rollCallCreateId, laoChannel)
    * call read('classpath:be/utils/simpleScenarios.feature@name=valid_lao')
    When eval frontend.send(badRollCallCreate)
    * json roll_err = frontend_buffer.takeTimeout(timeout)
    Then match roll_err contains deep {jsonrpc: '2.0', id: '#(rollCallCreateId)', error: {code: -4, description: '#string'}}
