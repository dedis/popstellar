...
Feature: Setup an Election
  ...
  Scenario: Setting up a valid election should succeed
    Given string electionSetupData = read('classpath:data/election/data/electionSetup/valid_election_setup_2_data.json')
    And string electionSetup = converter.publishМessageFromData(electionSetupData, id, channel)
    ...
    * json election_create = frontend_buffer.takeTimeout(timeout)
    Then match election_create contains deep {jsonrpc: '2.0', id: '#(id)', result: 0}
...
#    And string badElectionSetup = converter.publishМessageFromData(badElectionSetupData, id, channel)
#    * call read('classpath:be/utils/simpleScenarios.feature@name=valid_roll_call')
#    And eval frontend.send(badElectionSetup)
#    * json election_create_err = frontend_buffer.takeTimeout(timeout)
#    Then match election_create_err contains deep {jsonrpc: '2.0', id: '#(id)', error: {code: -4, description: '#string'}}
#
