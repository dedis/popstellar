Feature: Wallet
  ...
  * call read('classpath:fe/utils/constants.feature')
  ...
  Scenario: Open the app for the first time and see the wallet seed
    Given call read(PLATFORM_FEATURE) { name: "#(OPEN_APP)" }
  ...
  Scenario: Restore a wallet
    Given call read(PLATFORM_FEATURE) {name: "#(RESTORE_WALLET)", params: { seed: 'present guilt frost screen fabric rotate citizen decide have message chat hood' } }
    When waitFor(lao_join_button)
    Then screenshot()
