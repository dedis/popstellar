...
let mockedFn: (m: MessageData) => void = (m: MessageData) => {};
...
function checkDataCreateLao(obj: MessageData) {
  ...
  expect(data.creation).toBeNumber();
...
function checkDataCreateMeeting(obj: MessageData) {
  ...
  expect(data.start).toBeGreaterThanOrEqual(data.creation.valueOf());
...
function checkDataCreateRollCall(obj: MessageData) {
  ...
  if ('start' in data) {
    ...
    expect(data.start).toBeGreaterThan(0);
    // @ts-ignore
    expect(data.start.valueOf() + 1).toBeGreaterThan(data.creation);
  }

  if ('scheduled' in data) {
    expect(data.scheduled).toBeNumber();
    expect(data.scheduled).toBeGreaterThan(0);
    // @ts-ignore
    expect(data.scheduled.valueOf() + 1).toBeGreaterThan(data.creation);
  }
...
function checkDataOpenRollCall(obj: MessageData) {
  ...
  // check id
  const expected = Hash.fromStringArray('R', OpenedLaoStore.get().id.toString(), (444).toString(), 'r-cName'); // 444 and r-cName are for now hardocded in the APi
  expect(data.id).toEqual(expected);
}
