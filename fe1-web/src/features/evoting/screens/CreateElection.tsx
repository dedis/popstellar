...
const createElection = (
  ...
  const electionId = Hash.fromStringArray(EventTags.ELECTION, laoId, now.toString(), electionName);

  // compute the id for all questions and add the write_in property
  const questionsWithId = questions.map((item) =>
    Question.fromState({
      id: Hash.fromStringArray(EventTags.QUESTION, electionId, item.question).toString(),
