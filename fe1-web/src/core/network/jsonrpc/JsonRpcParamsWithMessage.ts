...
export class JsonRpcParamsWithMessage extends JsonRpcParams {
  ...
  public static fromJson(obj: any): JsonRpcParamsWithMessage {
    ...
    // Here we assign the channel to the messages for all incoming messages, then we can use
    // it in the handler for each messages (especially important for election result messages
    // as it has no election id in the messages, we need to get it from the channel)
