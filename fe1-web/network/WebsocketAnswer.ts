...
import { getStore, ActionOpenedLaoReducer } from 'store';
...
const hasPendingQuery = (idx) => WebsocketLink.getPendingProperties().has(idx);

/** returns the query with id idx from the pending queue */
const getPendingQuery = (idx) => WebsocketLink.getPendingProperties().get(idx);

/** sets or modifies the query with id idx in/from the pending queue to value */
const setPendingQuery = (idx, value) => WebsocketLink.getPendingProperties().set(idx, value);

/** removes the query with id idx from the pending queue */
const delPendingQuery = (idx) => WebsocketLink.getPendingProperties().delete(idx);

/** handles a positive server answer */
const handlePositiveServerAnswer = (obj, isPropagation = false) => {
  const answer = isPropagation ? obj : getPendingQuery(obj.id);
    ...
    case objects.LAO:
      if (answer.requestAction === actions.CREATE) {
        ...
        if (!validateDataCreateLao(jsonMessage.params.message.data)) {
          throw new Error(
            '(handlePositiveServerAnswer) parsed dataCreateLao doesn\'t conform to JsonSchema',
          );
        }

        // store new LAO
        // FIXME: use OpenedLaoStore.store()
        getStore().dispatch({
          type: ActionOpenedLaoReducer.SET_OPENED_LAO,
          value: jsonMessage.params.message.data,
        });
        ...
        if (!validateDataUpdateLao(jsonMessageData)) {
          throw new Error(
            '(handlePositiveServerAnswer) parsed dataUpdateLao doesn\'t conform to JsonSchema',
          );
        }
        ...
        // store updated LAO
        // FIXME: use OpenedLaoStore.store()
        getStore().dispatch({ type: ActionOpenedLaoReducer.SET_OPENED_LAO, value: updatedLao });
        RootNavigation.navigate(STRINGS.app_navigation_tab_organizer);
  ...
  if (!isPropagation) delPendingQuery(obj.id);
};

/** handles a negative server answer */
const handleNegativeServerAnswer = (obj) => {
  const { error } = obj;
  ...
  const query = getPendingQuery(obj.id);
  const { retryCount } = getPendingQuery(obj.id);
  ...
  if (retryCount < MAX_QUERY_RETRIES) {
    setPendingQuery(obj.id, newQuery);
    ...
    delPendingQuery(obj.id);
  }
};

/** handles a propagate message */
const handlePropagateMessage = (queryParams) => {
  ...
  )) throw new Error('(handlePropagateMessage) message fields missing or wrongly formatted');
  ...
  try {
    ...
    throw new Error('(handlePropagateMessage) message fields are not valid base64 strings');
  }
  ...
  ) throw new Error('(handlePropagateMessage) message signature is incorrect');

  // check that the message_id is correct
  if (
    decodedMessage.message_id !== hashStrings(decodedMessage.data, decodedMessage.signature)
  ) throw new Error('(handlePropagateMessage) message_id is incorrect');
  ...
  ) throw new Error('(handlePropagateMessage) data\'s object/action field is missing or wrongly formatted');

  if (!validateData(dataObject)) {
    throw new Error(
      '(handlePositiveServerAnswer) parsed data from propagation message doesn\'t conform to JsonSchema',
    );
  }
  ...
  const requestObject = (function lambda(object) {
    switch (object) {
      case objects.LAO:
        return objects.LAO;
      case objects.MEETING:
  ...
  const requestAction = (function lambda(action) {
    switch (action) {
      case actions.CREATE:
...
const handleServerAnswer = (message) => {
  ...
  if (!Object.prototype.hasOwnProperty.call(obj, answerProperties.ID)) {
    if (Object.prototype.hasOwnProperty.call(obj, 'method')
      ...
      if (validatePropagateMessage(obj)) {
        handlePropagateMessage(obj.params);
        return;
      }
  ...
  if (Object.prototype.hasOwnProperty.call(obj, answerProperties.RESULT)) {
    ...
    if (obj.result === 0) {
      ...
      handlePositiveServerAnswer(obj);
    } else {
      throw new Error(
    ...
    handleNegativeServerAnswer(obj);
  }
};
