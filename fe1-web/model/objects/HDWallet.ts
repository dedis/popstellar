...
export class HDWallet {
  ...
  public async initialize(mnemonic: string): Promise<boolean> {
    ...
    this.encryptedSeed = await platformCrypto.encrypt(seed);
    WalletStore.store(this.toState());
    return true;
  }
  ...
  public async getDecryptedSeed(): Promise<Uint8Array> {
    ...
    const storedSeed = base64url.toBuffer(encodedStoredSeed);
    const plaintext: ArrayBuffer = await platformCrypto.decrypt(storedSeed);
  ...
  public static async fromState(encryptedSerializedSeed: string): Promise<HDWallet> {
    const seed: Uint8Array = base64url.toBuffer(encryptedSerializedSeed);
  ...
  public toState(): string {
    return base64url.toBase64(Buffer.from(this.encryptedSeed));
  }
  ...
  private async generateKeyFromPath(path: string): Promise<KeyPair> {
    const seedArray = await this.getDecryptedSeed()
    ...
    const token = new KeyPair({
      ...
      privateKey: new PrivateKey(Base64UrlData.fromBuffer(privKey).valueOf()),
    });
