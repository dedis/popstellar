...
class JsonRpcRequestSuite extends FunSuite with Matchers {
    ...
    test("setDecodedData sets data as intended"){
        ...
        val rpcReq2Set: JsonRpcRequest = JsonRpcRequest(rpc, methodType, paramsWithMessageToModify, id)
        rpcReq2Set.setDecodedData(decodedData)

        rpcReq2Set.getDecodedData should equal (Some(decodedData))
    }
