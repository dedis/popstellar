...
class ResultSuite extends FunSuite with Matchers {
  ...
  test("Constructor/apply works for Result as expected") {
    ...
    RESULT_1.challenge_message should equal(CHALLENGE)
    ...
    RESULT_2.challenge_message should equal(CHALLENGE)
  ...
  test("The system should only accept a status of success or failure ") {
    an[IllegalArgumentException] should be thrownBy FederationResult("unknown", ORGANIZER, CHALLENGE)
  }
}
