...
import ch.epfl.pop.decentralized.{ConnectionMediator, FailingToyDbActor, Monitor, ToyDbActor}
...
class MonitorSuite extends TestKit(ActorSystem("MonitorSuiteActorSystem")) with FunSuiteLike with Matchers with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    println("beforeAll Monitor")
    PublishSubscribe.buildGraph(ActorRef.noSender, toyDbActorRef, ActorRef.noSender, MessageRegistry(), ActorRef.noSender, ActorRef.noSender, ActorRef.noSender, isServer = false)

  }

  override def afterAll(): Unit = {
    // Stops the test actor system
    println("afterAll Monitor")
