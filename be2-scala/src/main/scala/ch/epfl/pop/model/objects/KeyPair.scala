 ...
 * @param privateKey key to use for encryption
 * @param publicKey key to use for decryption
 *
 * @note Warning: this is not intended to be used for signing purpose !! (see Signature.scala for such usage)
 */
...
object KeyPair {
   ...
   * @return a KeyPair holding both keys generated
   *
   * @note Warning: this is not intended to be used for signing purpose !! (see Signature.scala for such usage)
   */
  def apply(): KeyPair = {
