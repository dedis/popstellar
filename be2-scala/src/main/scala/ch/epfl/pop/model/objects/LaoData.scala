...
/** @param owner
  *   : the LAO owner's public key, with which the signed messages can be authenticated
  * @param attendees
  *   : list of the last roll call attendees
  * @param privateKey
  *   : the LAO's own private key, used to sign messages
  * @param publicKey
  *   : the LAO's own public key, used to sign messages
  * @param witnesses
  *   : the LAO's list of witnesses
  * @param address
  ...
  def updateWith(message: Message, adr: Option[String]): LaoData = {
    message.decodedData.fold(this) {
      case call: CloseRollCall => adr match {
          case Some(str) => LaoData(owner, owner :: call.attendees, privateKey, publicKey, witnesses, str)
          case _         => LaoData(owner, owner :: call.attendees, privateKey, publicKey, witnesses, address)
        }
