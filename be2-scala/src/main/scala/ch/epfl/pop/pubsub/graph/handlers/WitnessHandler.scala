...
case object WitnessHandler extends MessageHandler {
  ...
  def handleWitnessMessage(rpcMessage: JsonRpcRequest): GraphMessage = {
    ...
    val ask = dbActor.ask(ref => DbActorNew.Read(rpcMessage.getParamsChannel, messageId, ref)).map {
      case Some(message: Message) =>
        val askWrite = dbActor.ask(ref => DbActorNew.Write(
        ...
        Await.result(askWrite, DbActorNew.getDuration)
