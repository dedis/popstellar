...
func handleChannel(params handlerParameters, channelType string, msg message.Message) *answer.Error {
	...
	if err != nil {
		return answer.NewInvalidMessageFieldError("failed to decode data string: %v", err).Wrap("handleChannel")
	}

	publicKeySender, err := base64.URLEncoding.DecodeString(msg.Sender)
	if err != nil {
		return answer.NewInvalidMessageFieldError("failed to decode public key string: %v", err).Wrap("handleChannel")
	}

	signatureBytes, err := base64.URLEncoding.DecodeString(msg.Signature)
	if err != nil {
		return answer.NewInvalidMessageFieldError("failed to decode signature string: %v", err).Wrap("handleChannel")
	}

	err = schnorr.VerifyWithChecks(crypto.Suite, publicKeySender, dataBytes, signatureBytes)
	if err != nil {
		return answer.NewInvalidMessageFieldError("failed to verify signature : %v", err).Wrap("handleChannel")
	}
	...
	if err == nil {
		return answer.NewInvalidActionError("message %s was already received", msg.MessageID).Wrap("handleChannel")
	}
