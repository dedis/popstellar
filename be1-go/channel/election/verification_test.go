...
func TestVerify_CastVote(t *testing.T) {
	...
	buf, err := os.ReadFile(filepath.Join(relativeExamplePath, "vote_cast_vote", "vote_cast_vote.json"))
	...
	getTestBadExample := func(file string) func(*testing.T) {
		return func(t *testing.T) {
			...
			buf, err = os.ReadFile(filepath.Join(relativeExamplePath, "vote_cast_vote", file))
			require.NoError(t, err)
...
func TestVerify_ElectionEnd(t *testing.T) {
	...
	// read the valid example file
	buf, err := os.ReadFile(filepath.Join(relativeExamplePath, "election_end", "election_end.json"))
	require.NoError(t, err)
	...
	getTestBadExample := func(file string) func(*testing.T) {
		return func(t *testing.T) {
			// read the bad example file
			buf, err = os.ReadFile(filepath.Join(relativeExamplePath, "election_end", file))
			require.NoError(t, err)
