...
const (
	...
	sKPath = "../../crypto/popcha.rsa"
	//path to RSA public key
	pKPath = "../../crypto/popcha.rsa.pub"
...
func (c *Channel) auhenticateUser(msg message.Message, msgData interface{},
	...
	popChaPath := strings.Join([]string{"/response", c.channelID, data.ClientID, data.Nonce}, "/")
...
func loadRSAKeys() (*rsa.PrivateKey, *rsa.PublicKey, error) {
	privBytes, err := os.ReadFile(sKPath)
	if err != nil {
		return nil, nil, err
	}
