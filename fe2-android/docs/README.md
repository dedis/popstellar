...
On a higher level of abstraction, the frontend may publish messages or subscribe for messages on a _channel_. You may
...
- The **Model** consists of the model classes stored in the repositories.
  ...
  For now, the persistence in the application is done via storing the wallet main seed, the server address and the list of subscribed channels. Upon restoring an application state, a catch-up is sent to the backend for each channel. 
