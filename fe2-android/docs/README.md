...
The `LAONetworkManager` then hands the message over to the `MessageHandler`, which will in turn use the application's `DataRegistry` to pass the
...
##### Warning
In the current version of the build tool, more needs to be done when annotating a class with `@AndroidEntryPoint`.\

Here is an example :
```java
public class MyActivity extends ActivityCompat {}
```
becomes
```java
@AndroidEntryPoint(ActivityCompat.class)
public class MyActivity extends Hilt_MyActivity {}
```

Hilt_MyActivity does not exist until the next build. It is generated by Hilt. So the compilation error is normal. Build the project to fix it.\
The solution was found here : https://github.com/google/dagger/issues/1956#issuecomment-650326353
