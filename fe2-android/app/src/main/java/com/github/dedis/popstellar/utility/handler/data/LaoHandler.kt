...
class LaoHandler @Inject constructor(
    ...
    fun handleUpdateLao(context: HandlerContext, updateLao: UpdateLao) {
        ...
        val message: WitnessMessage = if (updateLao.name != laoView.name) {
    ...
    fun handleStateLao(context: HandlerContext, stateLao: StateLao) {
        ...
        lao.pendingUpdates
            .removeIf { pendingUpdate: PendingUpdate -> pendingUpdate.modificationTime <= targetTime }
        laoRepo.updateLao(lao)
        laoRepo.updateNodes(channel)
    }
