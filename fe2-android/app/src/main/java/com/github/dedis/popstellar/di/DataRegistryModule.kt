...
object DataRegistryModule {
    ...
    fun provideDataRegistry(
            ...
            witnessMessageHandler: WitnessingHandler): DataRegistry {
        val builder = DataRegistry.Builder()

        // Lao
        builder
                .add(Objects.LAO, Action.CREATE, CreateLao::class.java) { context: HandlerContext?, createLao: CreateLao? -> laoHandler.handleCreateLao(context, createLao) }
                .add(Objects.LAO, Action.UPDATE, UpdateLao::class.java) { context: HandlerContext?, updateLao: UpdateLao? -> laoHandler.handleUpdateLao(context, updateLao) }
                .add(Objects.LAO, Action.STATE, StateLao::class.java) { context: HandlerContext?, stateLao: StateLao? -> laoHandler.handleStateLao(context, stateLao) }
                .add(Objects.LAO, Action.GREET, GreetLao::class.java) { context: HandlerContext?, greetLao: GreetLao? -> laoHandler.handleGreetLao(context, greetLao) }

        // Meeting
        builder
                .add(Objects.MEETING, Action.CREATE, CreateMeeting::class.java) { context: HandlerContext?, createMeeting: CreateMeeting? -> meetingHandler.handleCreateMeeting(context, createMeeting) }
                .add(Objects.MEETING, Action.STATE, StateMeeting::class.java) { context: HandlerContext?, stateMeeting: StateMeeting? -> meetingHandler.handleStateMeeting(context, stateMeeting) }

        // Message
        builder.add(Objects.MESSAGE, Action.WITNESS,
                WitnessMessageSignature::class.java) { context: HandlerContext?, witnessMessageSignature: WitnessMessageSignature? -> witnessMessageHandler.handleWitnessMessageSignature(context, witnessMessageSignature) }

        // Roll Call
        builder
                .add(Objects.ROLL_CALL, Action.CREATE, CreateRollCall::class.java) { context: HandlerContext?, createRollCall: CreateRollCall? -> rollCallHandler.handleCreateRollCall(context, createRollCall) }
                .add(Objects.ROLL_CALL, Action.OPEN, OpenRollCall::class.java) { context: HandlerContext?, openRollCall: OpenRollCall? -> rollCallHandler.handleOpenRollCall(context, openRollCall) }
                .add(Objects.ROLL_CALL, Action.REOPEN, OpenRollCall::class.java) { context: HandlerContext?, openRollCall: OpenRollCall? -> rollCallHandler.handleOpenRollCall(context, openRollCall) }
                .add(Objects.ROLL_CALL, Action.CLOSE, CloseRollCall::class.java) { context: HandlerContext?, closeRollCall: CloseRollCall? -> rollCallHandler.handleCloseRollCall(context, closeRollCall) }

        // Election
        builder
                .add(Objects.ELECTION, Action.SETUP, ElectionSetup::class.java) { context: HandlerContext?, electionSetup: ElectionSetup? -> electionHandler.handleElectionSetup(context, electionSetup) }
                .add(Objects.ELECTION, Action.OPEN, ElectionOpen::class.java) { context: HandlerContext?, electionOpen: ElectionOpen? -> electionHandler.handleElectionOpen(context, electionOpen) }
                .add(Objects.ELECTION, Action.CAST_VOTE, CastVote::class.java) { context: HandlerContext?, castVote: CastVote? -> electionHandler.handleCastVote(context, castVote) }
                .add(Objects.ELECTION, Action.END, ElectionEnd::class.java) { context: HandlerContext?, electionEnd: ElectionEnd? -> electionHandler.handleElectionEnd(context, electionEnd) }
                .add(Objects.ELECTION, Action.RESULT, ElectionResult::class.java) { context: HandlerContext?, electionResult: ElectionResult? -> electionHandler.handleElectionResult(context, electionResult) }
                .add(Objects.ELECTION, Action.KEY, ElectionKey::class.java) { context: HandlerContext?, electionKey: ElectionKey? -> electionHandler.handleElectionKey(context, electionKey) }

        // Consensus
        builder
                .add(Objects.CONSENSUS, Action.ELECT, ConsensusElect::class.java) { context: HandlerContext?, consensusElect: ConsensusElect? -> consensusHandler.handleElect(context, consensusElect) }
                .add(Objects.CONSENSUS, Action.ELECT_ACCEPT,
                        ConsensusElectAccept::class.java) { context: HandlerContext?, consensusElectAccept: ConsensusElectAccept? -> consensusHandler.handleElectAccept(context, consensusElectAccept) }
                .add(Objects.CONSENSUS, Action.PREPARE, ConsensusPrepare::class.java) { context: HandlerContext?, data: ConsensusPrepare? -> consensusHandler.handleBackend(context, data) }
                .add(Objects.CONSENSUS, Action.PROMISE, ConsensusPromise::class.java) { context: HandlerContext?, data: ConsensusPromise? -> consensusHandler.handleBackend(context, data) }
                .add(Objects.CONSENSUS, Action.PROPOSE, ConsensusPropose::class.java) { context: HandlerContext?, data: ConsensusPropose? -> consensusHandler.handleBackend(context, data) }
                .add(Objects.CONSENSUS, Action.ACCEPT, ConsensusAccept::class.java) { context: HandlerContext?, data: ConsensusAccept? -> consensusHandler.handleBackend(context, data) }
                .add(Objects.CONSENSUS, Action.LEARN, ConsensusLearn::class.java) { context: HandlerContext?, consensusLearn: ConsensusLearn? -> consensusHandler.handleLearn(context, consensusLearn) }
                .add(Objects.CONSENSUS, Action.FAILURE, ConsensusFailure::class.java) { context: HandlerContext?, failure: ConsensusFailure? -> consensusHandler.handleConsensusFailure(context, failure) }

        // Social Media
        builder // Chirps
                .add(Objects.CHIRP, Action.ADD, AddChirp::class.java) { context: HandlerContext?, addChirp: AddChirp? -> chirpHandler.handleChirpAdd(context, addChirp) }
                .add(Objects.CHIRP, Action.NOTIFY_ADD, NotifyAddChirp::class.java, null)
                .add(Objects.CHIRP, Action.DELETE, DeleteChirp::class.java) { context: HandlerContext?, deleteChirp: DeleteChirp? -> chirpHandler.handleDeleteChirp(context, deleteChirp) }
                .add(Objects.CHIRP, Action.NOTIFY_DELETE, NotifyDeleteChirp::class.java, null) // Reactions
                .add(Objects.REACTION, Action.ADD, AddReaction::class.java) { context: HandlerContext?, addReaction: AddReaction? -> reactionHandler.handleAddReaction(context, addReaction) }
                .add(Objects.REACTION, Action.DELETE, DeleteReaction::class.java) { context: HandlerContext?, deleteReaction: DeleteReaction? -> reactionHandler.handleDeleteReaction(context, deleteReaction) }

        // Digital Cash
        builder.add(Objects.COIN, Action.POST_TRANSACTION,
                PostTransactionCoin::class.java) { context: HandlerContext?, postTransactionCoin: PostTransactionCoin? -> transactionCoinHandler.handlePostTransactionCoin(context, postTransactionCoin) }
