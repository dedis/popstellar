...
class MessageRepository @Inject constructor(appDatabase: AppDatabase, application: Application) {
     ...
     * other repositories. The messages for which we store the actual content can be found here [ ][com.github.dedis.popstellar.model.network.method.message.data.Action.isStoreNeededByAction]
     */
                    ...
                    { messageEntities: List<MessageEntity> ->
                        messageEntities.forEach(
                            Consumer { msg: MessageEntity ->
                                messageCache.put(
                                    msg.messageId,  // Cache doesn't accept null as value, so an empty message is used
