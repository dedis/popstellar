...
@Singleton
class DigitalCashRepository @Inject constructor(
    ...
    private fun getLaoTransactions(laoId: String): LaoTransactions {
        ...
        return transactionsByLao.computeIfAbsent(laoId) {
            LaoTransactions(
                laoId,
                this
            )
    ...
    private class LaoTransactions(
        ...
        fun initializeDigitalCash(attendees: List<PublicKey>) {
            ...
            repository.disposables.add(
                repository.transactionDao
                    ...
                    .subscribe(
                        {
                            Timber.tag(TAG)
                                .d("Cleared the transactions in the db for lao %s", laoId)
                        }
                    ) { err: Throwable ->
            ...
            transactionsSubject.values.forEach(Consumer { observer: Subject<List<TransactionObject>> ->
            ...
            repository.disposables.add(
                repository.hashDao
                    ...
                    .subscribe(
                        {
                            Timber.tag(TAG)
                                .d("Cleared the hash dictionary in the db for lao %s", laoId)
            ...
            for (current in getReceiversTransaction(transaction)) {
                ...
                if (transactionList == null) {
                    ...
                    if (transactionsSubject.containsKey(current)) {
                        // Updating subject
                        transactionsSubject[current]
                            ?.toSerialized()
                            ?.onNext(ArrayList(transactionList))
                    } else {
                        // Creating new subject
                        transactionsSubject[current] =
                            BehaviorSubject.createDefault(ArrayList(transactionList))
                    }
