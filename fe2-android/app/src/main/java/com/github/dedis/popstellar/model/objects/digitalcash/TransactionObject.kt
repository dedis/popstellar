...
class TransactionObject(
    ...
    init {
        this.inputs = inputs?.let { ArrayList(it) }?.let { Collections.unmodifiableList(it) }!!
        this.outputs = outputs?.let { ArrayList(it) }?.let { Collections.unmodifiableList(it) }!!
    }
    ...
    val sendersTransaction: List<PublicKey?>
        ...
        get() = inputs.stream().map { input: InputObject -> input.script.pubKey }.collect(Collectors.toList())
    val receiversHashTransaction: List<String>
        ...
        get() = outputs.stream().map(OutputObject::pubKeyHash).collect(Collectors.toList())

    /**
     * Function that gives the Public Keys of the Outputs
     *
     * @param mapHashKey Map<String></String>,PublicKey> dictionary public key by public key hash
    ...
    fun getReceiversTransaction(mapHashKey: Map<String?, PublicKey?>): List<PublicKey> {
        ...
        for (transactionHash in receiversHashTransaction) {
            val pub = mapHashKey[transactionHash]
    ...
    fun getSumForUser(user: PublicKey?): Long {
        ...
        if (isCoinBaseTransaction) {
            ...
            return outputs.stream()
                    ...
                    .mapToLong { obj -> obj.value }
                    .sum()
        }
        var sum = 0
        if (isSender(user)) {
            // if the user is sender, we subtract the value of all output
            sum -= outputs.stream().mapToLong { obj -> obj.value }.sum().toInt()
        }
    ...
    val isCoinBaseTransaction: Boolean
         ...
         */
        get() = sendersTransaction.size == 1 && inputs[0].txOutHash == TX_OUT_HASH_COINBASE && inputs[0].txOutIndex == 0
