 ...
 *
 *
 * The superclass works as main connection to the first server, the others are instantiated in
 * the connectionMap.
 */
class MultiConnection(
    /** Caller method for the connectionFactory to instantiate connections  */
    private val connectionProvider: Function<String, Connection>, url: String
) : Connection(
    ...
    fun connectToPeers(peerAddressList: List<PeerAddress>): Boolean {
        ...
        val newPeers = peerAddressList.stream()
            .filter { peer: PeerAddress -> !connectionMap.containsKey(peer) }
            .collect(Collectors.toList())
        if (newPeers.isEmpty()) {
            return false
        }
        newPeers.forEach(Consumer { p: PeerAddress ->
            connectionMap[p] = connectionProvider.apply(p.address)
        })
        return true
    }
