...
public class LaoHandler {
  ...
  public static boolean handleLaoMessage(
    ...
    Log.d(TAG, "handle LAO message id=" + messageId);
  ...
  public static boolean handleStateLao(
    ...
    String modificationId = stateLao.getModificationId();

    Lao lao = laoRepository.getLaoByChannel(channel);
    if (!laoRepository.getMessageById().containsKey(modificationId)) {
    ...
    for (PublicKeySignaturePair pair : stateLao.getModificationSignatures()) {
      if (!Signature.verifySignature(modificationId, pair.getWitness(), pair.getSignature())) {
        return true;
      }
    }
    ...
    lao.setModificationId(modificationId);
  ...
  public static WitnessMessage updateLaoWitnessesWitnessMessage(
    ...
    // TODO What is this ? Why retrieving the end of a set ? A set does not have an order...
