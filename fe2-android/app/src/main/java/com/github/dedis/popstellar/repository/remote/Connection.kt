...
open class Connection {
    ...
    constructor(
        url: String, laoService: LAOService, manualState: BehaviorSubject<Lifecycle.State>
    ) {
        ...
        disposables.add(
            laoService
                ...
                .subscribe({ t: GenericMessage? ->
                    messagesSubject.onNext(
                        t!!
                    )
                }) { t: Throwable? ->
                    messagesSubject.onError(
                        t!!
                    )
                })
        ...
        disposables.add(
            laoService
                ...
                .subscribe(
                    ...
                    { err: Throwable? ->
                        Timber.tag(TAG).d(err, "Error in connection %s", url)
                    })
        )
    }
