...
class PublicKey : Base64URLData {
    ...
    fun verify(signature: Signature, data: Base64URLData): Boolean {
        return try {
            ...
            Timber.tag(TAG).d("failed to verify witness signature %s", e.message)
            false
        }
    }
    ...
    fun computeHash(): String {
        ...
        return try {
            ...
            val hash = digest.digest(getData())
            Base64.getUrlEncoder().encodeToString(Arrays.copyOf(hash, 20))
