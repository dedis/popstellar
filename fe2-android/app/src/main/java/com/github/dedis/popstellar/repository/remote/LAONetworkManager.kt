...
class LAONetworkManager(
    ...
    private fun resubscribeToChannelOnReconnection() {
        disposables.add(
            multiConnection
                ...
                .subscribeOn(schedulerProvider.io()) // Filter out events that are not related to a reconnection
                .filter { event: WebSocket.Event? -> event is WebSocket.Event.OnConnectionOpened<*> }
                // Subscribe to the stream and when a connection event is received, send a subscribe message
                ...
                .subscribe(
                    ...
                    { error: Throwable ->
                        Timber.tag(TAG).d(error, "Error on resubscription")
                    })
        )
    }
    ...
    private fun processIncomingMessages() {
        disposables.add(
            Observable.merge( // Normal message received over the wire
                multiConnection.observeMessage(),  // Packets that could not be processed (maybe due to a reordering),
    ...
    override fun subscribe(channel: Channel): Completable {
        ...
        return request(subscribe) // This is used when reconnecting after a lost connection
            ...
            .doOnError { error: Throwable ->
                Timber.tag(TAG).d(error, "error in subscribe")
            }
    ...
    private fun handleBroadcast(broadcast: Broadcast) {
        ...
        try {
            ...
            Timber.tag(TAG)
                .e(e, "Error while handling received message, will try to reprocess it later")
    ...
    private fun request(query: Query): Single<Answer> {
        return multiConnection
            ...
            .map { obj: GenericMessage? -> obj as Answer } // This specific request has an id, only let the related Answer pass
            ...
            .flatMap { answer: Answer ->
                if (answer is Error) {
                    return@flatMap Single.error<Answer>(
            ...
            .observeOn(schedulerProvider.mainThread()) // Add a timeout to automatically dispose of the flow and end with a failure
