package com.github.dedis.popstellar.ui.home.wallet.stellar
 ...
 * the License.
 *
 * ----- [LICENSE](https://github.com/stellar/java-stellar-sdk/blob/master/LICENSE) [GITHUB
 * FILE](https://github.com/stellar/java-stellar-sdk/blob/master/src/main/java/org/stellar/sdk/SLIP10.java).
 * [GITHUB COMMIT](https://github.com/stellar/java-stellar-sdk/commit/a63d97970aff6121637235944be9c4ec48aa4fa2).
 */
object SLIP10 {
  private const val HMAC_SHA_512_ALGORITHM = "HmacSHA512"
   ...
   */
  @Throws(NoSuchAlgorithmException::class, ShortBufferException::class, InvalidKeyException::class)
  fun deriveEd25519PrivateKey(seed: ByteArray?, vararg indexes: Int): ByteArray {
    val iMacSha = ByteArray(64)
    val mac = Mac.getInstance(HMAC_SHA_512_ALGORITHM)

    // I = HMAC-SHA512(Key = bytes("ed25519 seed"), Data = seed)
    mac.init(
        SecretKeySpec("ed25519 seed".toByteArray(StandardCharsets.UTF_8), HMAC_SHA_512_ALGORITHM))
    mac.update(seed)
    mac.doFinal(iMacSha, 0)

    for (i in indexes) {
      // Key = Ir
      mac.init(SecretKeySpec(iMacSha, 32, 32, HMAC_SHA_512_ALGORITHM))
      // Data = 0x00
      mac.update(0x00.toByte())
      // Data += Il
      mac.update(iMacSha, 0, 32)
      // Data += ser32(i')
      mac.update((i shr 24 or 0x80).toByte())
      mac.update((i shr 16).toByte())
      mac.update((i shr 8).toByte())
      mac.update(i.toByte())
      // Write to I
      mac.doFinal(iMacSha, 0)
    }

    val ilMacSha = ByteArray(32)
