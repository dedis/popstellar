...
object MessageValidator {
    ...
    class MessageValidatorBuilder {
        ...
        fun validCreateMeetingId(
            id: String, laoId: String?, creation: Long, name: String?
        ): MessageValidatorBuilder {
            // If any of the arguments are empty or null this throws an exception
            require(
                id == Meeting.generateCreateMeetingId(
                    laoId,
                    creation,
                    name
                )
        ...
        fun validStateMeetingId(
            ...
            // If any of the arguments are empty or null this throws an exception
            require(
                id == Meeting.generateStateMeetingId(
                    laoId,
                    creation,
                    name
                )
        ...
        fun isBase64(input: String?, field: String): MessageValidatorBuilder {
            require(
                !(input == null || !BASE64_PATTERN.matcher(input).matches())
        ...
        fun isValidEmoji(input: String?, field: String): MessageValidatorBuilder {
            require(Reaction.ReactionEmoji.isSupported(input)) { "$field is not a supported unicode emoji" }
            return this
        }
        ...
        fun validUrl(input: String?): MessageValidatorBuilder {
            require(
                !(input == null || !URL_PATTERN.matcher(input).matches())
            ) { "Input is not a url" }
            return this
        }
        ...
        fun isValidPoPCHAUrl(input: String?, laoId: String): MessageValidatorBuilder {
            ...
            for (arg in REQUIRED_ARGUMENTS) {
                requireNotNull(uri.getQueryParameter(arg)) {
                    String.format(
                        "Required argument %s is missing in the URL.",
                        arg
                    )
                }
            }
            ...
            require(!Arrays.stream(REQUIRED_SCOPES)
        ...
        companion object {
            ...
            private val REQUIRED_ARGUMENTS = arrayOf(
