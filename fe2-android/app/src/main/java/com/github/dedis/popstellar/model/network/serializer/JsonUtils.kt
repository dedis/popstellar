...
object JsonUtils {
  ...
  fun testRPCVersion(obj: JsonObject) {
    if (!obj.has(JSON_RPC)) {
      throw JsonParseException("Unable to find jsonrpc version")
    }

    testRPCVersion(obj[JSON_RPC].asString)
  }
  ...
  fun loadSchema(resourcePath: String): JsonSchema {
    return schemas.computeIfAbsent(resourcePath) {
      FACTORY.getSchema(URI.create("resource:/$resourcePath"))
    }
  }
}
