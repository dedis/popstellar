 ...
 *
 *
 * *- What is a pending object?*<br></br>
 ...
 * know the pending object only from the message id.<br></br>
 * *- What does "enough" mean?*<br></br>
 ...
 * policy through the method hasRequiredSignatures().<br></br>
 * *- What does processing a pending object mean?*<br></br>
 * It means considering that object as valid, adding it to its own repository and doing other
 * processing job involved depending the object's type. <br></br>
 * <br></br>
 * *Example*<br></br>
 ...
 */
@Singleton
class WitnessingRepository @Inject constructor(
    ...
    fun addWitnesses(laoId: String, witnesses: Set<PublicKey>) {
        ...
        val witnessEntities =
            witnesses.stream().map { pk: PublicKey -> WitnessEntity(laoId, pk) }
                ...
                .subscribe(
                    {
                        Timber.tag(TAG)
                            .d(
                                "Persisted the pending entity corresponding to message %s",
                                messageId
                            )
                    }
                ) { err: Throwable ->
    ...
    private class LaoWitness(private val laoId: String, private val repo: WitnessingRepository) {
        ...
        private val witnessesSubject: Subject<Set<PublicKey>> = BehaviorSubject.createDefault(
        ...
        fun addWitnesses(witnesses: Set<PublicKey>) {
            ...
            witnessesSubject.toSerialized()
                .onNext(Collections.unmodifiableSet(HashSet(this.witnesses)))
        }
        ...
        fun deleteAcceptedMessages() {
            ...
            val idsToDelete = witnessMessages.values.stream()
                    ...
                    .subscribe(
                        {
                            Timber.tag(TAG)
                                .d(
                                    "Deleted accepted witness messages of lao %s from the disk",
                                    laoId
                                )
                        }
                    ) { err: Throwable ->
        ...
        private fun processPendingEntity(pendingEntity: PendingEntity) {
            when (pendingEntity.objectType) {
                Objects.ROLL_CALL -> addRollCallRoutine(
                ...
                Objects.ELECTION -> addElectionRoutine(
                    repo.electionRepository,
                    pendingEntity.election!!
                )

                Objects.MEETING -> addMeetingRoutine(
                    repo.meetingRepository, laoId, pendingEntity.meeting!!
                )
