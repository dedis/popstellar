...
class ConsensusHandler @Inject constructor(
    ...
    fun handleElectAccept(context: HandlerContext, consensusElectAccept: ConsensusElectAccept) {
        ...
        if (!electInstanceOpt.isPresent) {
            Timber.tag(TAG)
                .w("elect_accept for invalid messageId : %s", consensusElectAccept.messageId)
            throw InvalidMessageIdException(
                consensusElectAccept, consensusElectAccept.messageId
            )
        }
    ...
    fun handleConsensusFailure(context: HandlerContext, failure: ConsensusFailure) {
        val channel = context.channel
        Timber.tag(TAG)
            .d("handleConsensusFailure: channel: %s, id: %s", channel, failure.instanceId)
        val laoView = laoRepo.getLaoViewByChannel(channel)
