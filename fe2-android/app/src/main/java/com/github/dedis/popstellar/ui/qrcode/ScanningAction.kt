...
enum class ScanningAction(
    ...
    private val jsonFormatter: (Map<String, String>) -> String,
    private val scannerViewModelProvider: BiFunction<FragmentActivity, String?, QRCodeScanningViewModel>,
    ...
    ADD_WITNESS_AT_START(
        ...
        arrayOf(InputFieldConfig("Witness Public Key", true)),
        { inputs -> "{\"main_public_key\":\"${inputs["Witness Public Key"]}\"}" },
        { activity, _ -> HomeActivity.obtainWitnessingViewModel(activity) },
        { activity -> HomeActivity.obtainViewModel(activity) },
        { manager, _ -> HomeActivity.setCurrentFragment(manager, R.id.fragment_lao_create) { LaoCreateFragment() } },
        true
    ),
    ADD_WITNESS(
        R.string.qrcode_scanning_add_witness,
        ...
        { manager, _ -> LaoActivity.setCurrentFragment(manager, R.id.fragment_witnessing) { com.github.dedis.popstellar.ui.lao.witness.WitnessingFragment() } },
        true
    ),
    ADD_ROLL_CALL_ATTENDEE(
        ...
        R.string.manual_add_attendee_title,
        arrayOf(InputFieldConfig("PoP Token", true)),
        { inputs -> "{\"pop_token\":\"${inputs["PoP Token"]}\"}" },
        { activity, laoId -> LaoActivity.obtainRollCallViewModel(activity, laoId) },
        { activity -> LaoActivity.obtainViewModel(activity) },
        { manager, args -> LaoActivity.setCurrentFragment(manager, R.id.fragment_roll_call) { RollCallFragment.newInstance(args[0]) } },
        true
    ),
    ADD_LAO_PARTICIPANT(
        ...
        arrayOf(InputFieldConfig("Server Address", true), InputFieldConfig("Lao ID", true)),
        { inputs -> "{\"server\":\"${inputs["Server Address"]}\", \"lao\":\"${inputs["Lao ID"]}\"}" },
        { activity, _ -> HomeActivity.obtainViewModel(activity) },
        { activity -> HomeActivity.obtainViewModel(activity) },
        { manager, _ -> HomeActivity.setCurrentFragment(manager, R.id.fragment_home) { HomeFragment() } },
        false
    ),
    ADD_POPCHA(
        ...
        R.string.manual_popcha_add_title,
        arrayOf(InputFieldConfig("PoPCHA URL", true)),
        { inputs -> inputs["PoPCHA URL"] ?: "" },
        { activity, laoId -> LaoActivity.obtainPoPCHAViewModel(activity, laoId) },
        { activity -> LaoActivity.obtainViewModel(activity) },
        { manager, _ -> LaoActivity.setCurrentFragment(manager, R.id.fragment_popcha_home) { com.github.dedis.popstellar.ui.lao.popcha.PoPCHAHomeFragment() } },
        false
    );
