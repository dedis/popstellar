...
class Reaction : Copyable<Reaction?> {
    ...
    enum class ReactionEmoji(@JvmField val code: String) {
        UPVOTE("\uD83D\uDC4D"),
        DOWNVOTE("\uD83D\uDC4E"),
        HEART("❤️");
        ...
        companion object {
            ...
            fun isSupported(emoji: String): Boolean {
                return Arrays.stream(values())
                        .anyMatch { reactionEmoji: ReactionEmoji -> reactionEmoji.code == emoji }
            }
        }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return timestamp == reaction.timestamp && id == reaction.id && sender == reaction.sender
                && codepoint == reaction.codepoint && chirpId == reaction.chirpId
    }
