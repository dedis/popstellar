...
constructor(private val digitalCashRepo: DigitalCashRepository) {
  ...
  fun handlePostTransactionCoin(context: HandlerContext, postTransactionCoin: PostTransactionCoin) {
    ...
    while (iteratorInput.hasNext()) {
      ...
      val scriptInputObject =
          ScriptInputObject(current.script.type, current.script.pubkey, current.script.sig)
      inputs.add(InputObject(current.txOutHash, current.txOutIndex, scriptInputObject))
    }
