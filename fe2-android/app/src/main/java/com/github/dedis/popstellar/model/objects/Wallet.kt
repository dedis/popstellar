...
class Wallet @Inject constructor(@WalletKeyset keysetManager: AndroidKeysetManager) {
     ...
     */
    @Throws(KeyGenerationException::class, UninitializedWalletException::class, InvalidPoPTokenException::class)
    fun recoverKey(
            laoID: String, rollCallID: String, rollCallTokens: Set<PublicKey?>): PoPToken {
    ...
    fun exportSeed(): Array<String?> {
        ...
        val words = String(decryptedBytes, StandardCharsets.UTF_8)
    ...
    fun importSeed(words: String) {
        try {
            ...
            throw SeedValidationException(e)
    ...
    private fun storeEncrypted(mnemonicWords: String) {
        encryptedMnemonic = aead!!.encrypt(mnemonicWords.toByteArray(StandardCharsets.UTF_8), ByteArray(0))
        encryptedSeed = aead!!.encrypt(
                SeedCalculator().calculateSeed(java.lang.String.join("", mnemonicWords), ""), ByteArray(0))
    ...
    private fun generateKeyFromPath(path: String): PoPToken {
        ...
        val pathValueInt = Arrays.stream(path.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
                .skip(1) // remove the first element ('m')
                .mapToInt { s: String -> s.toInt() }
                .toArray()
        return try {
            // derive private and public key
            val privateKey = SLIP10.deriveEd25519PrivateKey(aead!!.decrypt(encryptedSeed, ByteArray(0)), *pathValueInt)
            val prK = Ed25519PrivateKeyParameters(privateKey, 0)
            val puK = prK.generatePublicKey()
            val publicKey = puK.encoded
            PoPToken(privateKey, publicKey)
        } catch (e: GeneralSecurityException) {
