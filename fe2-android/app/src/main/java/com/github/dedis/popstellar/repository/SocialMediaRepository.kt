...
class SocialMediaRepository @Inject constructor(
     ...
     *
     *
     * Its purpose is to hold data in a way that it is easier to handle and understand. It is also
    ...
    private class LaoChirps(
        ...
        private val chirpsSubject: Subject<Set<MessageID?>> = BehaviorSubject.createDefault(
            emptySet<MessageID>()
        )
        ...
        private val reactionSubjectsByChirpId =
            ConcurrentHashMap<MessageID, Subject<Set<Reaction>>>()
        ...
        fun add(chirp: Chirp) {
            ...
            reactionSubjectsByChirpId.putIfAbsent(
                chirp.id, BehaviorSubject.createDefault(HashSet())
            )
        ...
        fun addReaction(reaction: Reaction): Boolean {
            ...
            reactionSubjectsByChirpId[chirp.id]
                ?.toSerialized()
                ?.onNext(HashSet(chirpReactions))
            return true
        }
        ...
        fun deleteReaction(reactionId: MessageID): Boolean {
            ...
            val chirp = chirps[reaction.chirpId]
        ...
        fun getReactions(chirpId: MessageID): Observable<Set<Reaction>> {
            return reactionSubjectsByChirpId[chirpId]
                ?: throw UnknownChirpException(chirpId)
        }
