...
class JsonMessageGeneralSerializer : JsonSerializer<MessageGeneral>, JsonDeserializer<MessageGeneral> {
    @Throws(JsonParseException::class)
    override fun deserialize(
            json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): MessageGeneral {
        val jsonObject = context.deserialize<JsonMessageData>(json, JsonMessageData::class.java)
        val dataElement = JsonParser.parseString(String(jsonObject.data.data, StandardCharsets.UTF_8))
    ...
    override fun serialize(
            src: MessageGeneral, typeOfSrc: Type, context: JsonSerializationContext): JsonElement {
        val jsonObject = JsonMessageData(
                src.dataEncoded,
                src.sender,
                src.signature,
                src.messageId,
