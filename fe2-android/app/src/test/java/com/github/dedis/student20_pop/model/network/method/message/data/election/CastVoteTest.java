...
public class CastVoteTest {
  ...
  private ElectionVote electionVote1 = new ElectionVote(questionId1,
      new ArrayList<>(Arrays.asList(2, 1, 0)),
      writeInEnabled, write_in, electionId);
  private ElectionVote electionVote2 = new ElectionVote(questionId2,
      new ArrayList<>(Arrays.asList(0, 1, 2)),
      writeInEnabled, write_in, electionId);
  private List<ElectionVote> electionVotes = new ArrayList<>(
      Arrays.asList(electionVote1, electionVote2));
  ...
  public void isEqual() {
    ...
    assertNotEquals(castVote,
        new CastVote(new ArrayList<>(Arrays.asList(electionVote1)), electionId, laoId));
    assertNotEquals(castVote,
        new CastVote(new ArrayList<>(Arrays.asList(electionVote1)), "random", laoId));
    assertNotEquals(castVote,
