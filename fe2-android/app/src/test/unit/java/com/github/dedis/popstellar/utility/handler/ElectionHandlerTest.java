...
public class ElectionHandlerTest {
  ...
  private static final PlainVote VOTE1 = new PlainVote(QUESTION.id, 0, false, null, ELECTION_ID);
  private static final PlainVote VOTE2 = new PlainVote(QUESTION.id, 1, false, null, ELECTION_ID);
  private static final PlainVote VOTE3 = new PlainVote(QUESTION.id, 0, false, null, ELECTION_ID);
  ...
  private void handleElectionKey(Election election, String key)
    ...
    ElectionKey electionKey = new ElectionKey(election.id, key);
  ...
  private void handleElectionOpen(Election election)
      throws UnknownElectionException,
          ...
          NoRollCallException,
          UnknownWitnessMessageException {
    ElectionOpen electionOpen = new ElectionOpen(LAO.getId(), election.id, OPENED_AT);
  ...
  private MessageID handleCastVote(Vote vote, KeyPair senderKey, Long creation)
    ...
    CastVote castVote =
        new CastVote(singletonList(vote), OPEN_BALLOT_ELECTION.id, CREATE_LAO.id, creation);
  ...
  public void testHandleElectionSetup()
    ...
    Election election = electionRepo.getElectionByChannel(OPEN_BALLOT_ELECTION.channel);
    assertEquals(EventState.CREATED, election.getState());
    assertEquals(OPEN_BALLOT_ELECTION.id, election.id);
    assertEquals(OPEN_BALLOT_ELECTION.getElectionQuestions(), election.getElectionQuestions());
    assertEquals(OPEN_BALLOT_ELECTION.electionVersion, election.electionVersion);
    assertEquals(OPEN_BALLOT_ELECTION.getStartTimestamp(), election.getStartTimestamp());
    assertEquals(OPEN_BALLOT_ELECTION.creation, election.creation);
  ...
  public void testElectionKey()
    ...
    Election election = electionRepo.getElectionByChannel(OPEN_BALLOT_ELECTION.channel);
    assertEquals(ELECTION_KEY, election.electionKey);
  }
  ...
  public void testHandleElectionResult()
    ...
    handleCastVote(new PlainVote(QUESTION.id, 0, false, null, ELECTION_ID), SENDER_KEY, OPENED_AT);
    handleElectionEnd();
    handleElectionResults(results, OPEN_BALLOT_ELECTION.channel);

    Election election = electionRepo.getElectionByChannel(OPEN_BALLOT_ELECTION.channel);
    assertEquals(EventState.RESULTS_READY, election.getState());
    assertEquals(results, election.getResultsForQuestionId(QUESTION.id));
  }
  ...
  public void castVoteFailsOnPreviousMessageDataNull()
    ...
    Election prevElection = electionRepo.getElectionByChannel(OPEN_BALLOT_ELECTION.channel);
    Election updatedElection =
        prevElection.builder().updateMessageMap(SENDER, nullData.messageId).build();
  ...
  public void castVoteWithSecretBallotScenario()
    ...
    ElectionPublicKey pubKey = keys.encryptionScheme;
    Base64URLData encodedKey = new Base64URLData(pubKey.publicKey.toBytes());

    EncryptedVote vote1 = new EncryptedVote(QUESTION.id, "0", false, null, ELECTION_ID);
    EncryptedVote vote2 = new EncryptedVote(QUESTION.id, "1", false, null, ELECTION_ID);
